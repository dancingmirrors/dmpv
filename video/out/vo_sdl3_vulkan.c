...
static void set_fullscreen(struct vo *vo)
{
    ...
    SDL_WindowFlags old_flags = SDL_GetWindowFlags(p->window);
    int prev_fs = !!(old_flags & SDL_WINDOW_FULLSCREEN);
    if (fs == prev_fs)
        return;
    ...
    // For desktop fullscreen mode, size often stays the same
...
{
    struct priv *p = vo->priv;
    SDL_DisplayID display_id = SDL_GetDisplayForWindow(p->window);
...
static int preinit(struct vo *vo)
{
    ...
    // SDL3: SDL_CreateWindow signature changed - title and flags combined
    p->window = SDL_CreateWindow("dmpv (Vulkan)", 1280, 720,
...
static void wait_events(struct vo *vo, int64_t until_time_ns)
{
    ...
    while (SDL_WaitEventTimeout(&ev, timeout_ms)) {
        ...
        case SDL_EVENT_TEXT_INPUT: {
            ...
            int dmpv_mod = 0;
            if (sdl_mod & (SDL_KMOD_LCTRL | SDL_KMOD_RCTRL))
                dmpv_mod |= MP_KEY_MODIFIER_CTRL;
            if (sdl_mod & (SDL_KMOD_LALT | SDL_KMOD_RALT))
                dmpv_mod |= MP_KEY_MODIFIER_ALT;
            if (sdl_mod & (SDL_KMOD_LGUI | SDL_KMOD_RGUI))
...
static int control(struct vo *vo, uint32_t request, void *data)
{
    ...
    case VOCTRL_GET_UNFS_WINDOW_SIZE: {
        // Get non-fullscreen window size
        int *s = (int *)data;
        if (!p->window)
            return VO_FALSE;
        SDL_GetWindowSize(p->window, &s[0], &s[1]);
        return VO_TRUE;
    }
    
    case VOCTRL_SET_UNFS_WINDOW_SIZE: {
        // Set non-fullscreen window size
        int *s = (int *)data;
        if (!p->window)
            return VO_FALSE;
        SDL_SetWindowSize(p->window, s[0], s[1]);
        return VO_TRUE;
    }
    
    case VOCTRL_GET_FOCUSED: {
        // Get window focus state
        bool *focused = (bool *)data;
        if (!p->window)
            return VO_FALSE;
        SDL_WindowFlags flags = SDL_GetWindowFlags(p->window);
        *focused = !!(flags & SDL_WINDOW_INPUT_FOCUS);
        return VO_TRUE;
    }
...
const struct vo_driver video_out_sdl3_vulkan = {
    .description = "SDL 3 Renderer (Vulkan)",
    .name = "sdl3-vulkan",
