...
static void placebo_lock_queue(AVHWDeviceContext *ctx,
...
static void placebo_unlock_queue(AVHWDeviceContext *ctx,
                                 uint32_t queue_family, uint32_t index)
{
    struct queue_lock_ctx *lock_ctx = ctx->user_opaque;
...
static int vulkan_init(struct ra_hwdec *hw)
{
    ...
    bool has_internally_sync = false;
#endif
    MP_VERBOSE(hw, "Vulkan: Checking for extensions (%d total)\n",
...
#ifdef VK_KHR_internally_synchronized_queues
        else if (strcmp(vk->vulkan->extensions[i],
                        VK_KHR_INTERNALLY_SYNCHRONIZED_QUEUES_EXTENSION_NAME) == 0) {
            has_internally_sync = true;
        }
#endif
    }
...
AV_NOWARN_DEPRECATED(
    device_hwctx->lock_queue = placebo_lock_queue;
    device_hwctx->unlock_queue = placebo_unlock_queue;
)
...
#ifdef VK_KHR_internally_synchronized_queues
    if (has_internally_sync) {
        p->internally_synchronized_queues = true;
    }
#endif
