...
{
    struct vo_wayland_state *wl = data;
    struct vo_wayland_tranche *tranche, *tmp;
    wl_list_for_each_safe(tranche, tmp, &wl->tranche_list, link) {
                            ...
                            struct wl_array *indices)
{
    ...
    if (!tranche || !wl->format_map || !wl->format_size)
        return;

    const uint16_t *idx;
    uint32_t table_len = wl->format_size / sizeof(wayland_format);
    wl_array_for_each(idx, indices) {
        if (*idx >= table_len)
            continue;
        MP_TARRAY_APPEND(tranche, tranche->formats, tranche->num_formats,
                         wl->format_map[*idx]);
    }
}
...
static bstr generate_icc_profile_from_params(struct vo_wayland_state *wl)
{
    ...
    return gl_lcms_generate_profile_from_csp(NULL, wl->log, csp->primaries, csp->gamma);
}
#endif
...
bool vo_wayland_init(struct vo *vo)
{
    ...
    *wl = (struct vo_wayland_state) {
    ...
    if (!wl->display && !getenv("XDG_RUNTIME_DIR")) {
        const char *disp = getenv("WAYLAND_DISPLAY");
        if (disp && disp[0] != '/') {
            char path[PATH_MAX];
            snprintf(path, sizeof(path), "/run/user/%d/%s", (int)getuid(), disp);
            wl->display = wl_display_connect(path);
            if (wl->display)
                MP_VERBOSE(wl, "Connected to Wayland display via "
                           "%s.\n", path);
        }
    }
...
{
    struct vo_wayland_state *wl = vo->wl;
    ssize_t ignored = write(wl->wakeup_pipe[1], &(char){0}, 1);
    (void)ignored;
}
