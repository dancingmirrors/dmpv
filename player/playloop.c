    ...
    if (mpctx->seek.type) {
            ...
            mp_time_sec() - mpctx->start_timestamp < 0.3)
        {
            ...
            if (mpctx->video_status < STATUS_PLAYING)
                return;
            if (mp_time_sec() - mpctx->start_timestamp < 0.3) {
                // On A/V hr-seeks, always wait for the full result, to avoid
                // corner cases when seeking past EOF (we want it to determine
                // that EOF actually happened, instead of overwriting it with
                // the new seek).
