...
local function append_perfdata(s)
    ...
    -- ensure that the fixed title is one element
    s[#s+1] = format("%s%s%s%s{\\fn%s}{\\fs%s}%s{\\fs%s}",
                     o.nl, o.indent,
                     b("Frame Timings:"), o.prefix_sep, o.font, o.font_size,
    ...
    for _,frame in ipairs(sorted_keys(vo_p)) do  -- ensure fixed display order
        ...
        -- print the sum of each pass
        s[#s+1] = format(f, o.nl, o.indent, o.font_mono,
...
local function print_stats()
    local ass_content = default_stats()
