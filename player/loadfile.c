...
static void play_current_file(struct MPContext *mpctx)
{
    ...
    for (archive = 0; archive < 11; archive++) {
        if ((strncmp (str, mpctx->filename, 10) == 0)) {
            is_archive = true;
        }
    }

    if ((is_archive && is_image && mpctx->playlist->num_entries > 1 && mpctx->opts->image_correct_downscaling) ||
        (is_image && mpctx->opts->image_correct_downscaling)) {
